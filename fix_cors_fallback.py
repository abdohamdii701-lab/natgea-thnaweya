...
<head>
    ...
    <style>
        ...
        .auth-card h3 { font-size: 1.35rem; font-weight: 800; margin-bottom: 0.6rem; }
...
<body class="light-theme">
    ...
    <main class="container admin-main">
        ...
        <div id="authSection" class="auth-card">
            ...
            <p>أدخل كلمة سر الإدارة للوصول لسجل عمليات البحث المباشرة وعناوين الـ IP</p>

            <form id="authForm" style="display:flex; flex-direction:column; gap:1.2rem;">
                <div>
                    <input type="password" id="adminKeyInput" class="styled-input" placeholder="أدخل كلمة سر الإدارة (admin123)..." style="text-align:center; font-size:1rem;" required autofocus>
                </div>

                <button type="submit" class="btn btn-primary" style="justify-content:center; padding:12px; font-size:0.95rem;">🔓 دخول اللوحة</button>
            </form>

            <div id="corsNotice" class="cors-notice" style="display: none;">
                ⚠️ <strong>تنبيه اتصال (CORS):</strong><br>
                استضافة GitHub Pages لا تملك خادم بايثون لـ <code>app.py</code>.<br>
                • لرؤية البيانات على جهازك: افتح <a href="http://localhost:5000/admin.html" target="_blank" style="color:#ef4444; font-weight:800; text-decoration:underline;">http://localhost:5000/admin.html</a>.<br>
                • أو ارفع المشروع على Vercel ليعمل الرابط أونلاين مجاناً!
            </div>
        </div>
    ...
    <script>
        ...
        function startCountdown() {
            ...
            if (timerEl) timerEl.textContent = countdownTimer;
            ...
            timerInterval = setInterval(() => {
                ...
                if (tEl) tEl.textContent = countdownTimer;
            }, 1000);
        }

        // Multi-Endpoint Fallback Strategy (Localhost, Relative, Cloudflare/Vercel)
        async function fetchDashboardData() {
            const keyParam = encodeURIComponent(adminKey);
            const ts = Date.now();
            
            const candidateEndpoints = [
                `/api/admin/logs?key=${keyParam}&_t=${ts}`,
                `http://localhost:5000/api/admin/logs?key=${keyParam}&_t=${ts}`,
                `http://127.0.0.1:5000/api/admin/logs?key=${keyParam}&_t=${ts}`
            ];
